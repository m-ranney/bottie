...
def generate_meal_plan(num_days, meal_goal):
    response = openai.Completion.create(
        ...
        prompt=f"Please provide a meal plan for {num_days} days, with a focus on {meal_goal} meals. For each day, suggest breakfast, lunch, and dinner options. Be as descriptive as possible. Return your response where each day is labeled 'Day 1 Breakfast: ', 'Day 1 Lunch: ', 'Day 1 Dinner: ', 'Day 2 Breakfast: '... in a bulleted list format. Add some variety to the meal suggestions. Apply the ability to buy ingredients that can be used in multiple selected dishes for more efficient grocery shopping.",
        temperature=0.7,
        max_tokens=300,
        top_p=1,
        frequency_penalty=0,
