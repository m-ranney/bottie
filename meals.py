...
def generate_meal_plan(num_days, meal_goal):
    response = openai.Completion.create(
        ...
        prompt=f"Please provide a meal plan for {num_days} days, with a focus on {meal_goal} meals. Suggest meals for breakfast, lunch, and dinner. Be as descriptive as possible. Include a '.' as a delimiter after each meal, and only include meals using the following format template for your response:---BEGIN FORMAT TEMPLATE---BREAKFAST: (Breakfast Meal). (Breakfast Meal). (Breakfast Meal). etc..., LUNCH: (Lunch Meal). (Lunch Meal). (Lunch Meal). etc..., DINNER: (Dinner Meal). (Dinner Meal). (Dinner Meal). etc...---END FORMAT TEMPLATE--- Add some variety to the meal suggestions. Apply the ability to buy ingredients that can be used in multiple selected dishes for more efficient grocery shopping. I will be parsing the output based on the format template, so do not include any other content that would cause any errors in formatting. ",
        temperature=1,
        max_tokens=400,
        top_p=1,
        frequency_penalty=0,
